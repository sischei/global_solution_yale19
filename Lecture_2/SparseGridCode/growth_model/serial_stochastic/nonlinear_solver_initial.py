...
def initial(k_init, theta_init, n_agents):
    # IPOPT PARAMETERS below 
    nvars=3*n_agents
    N=nvars #nvars   # number of vars
    ...
    def eval_f(X):
        return EV_F(X, k_init, theta_init, n_agents)
    
    def eval_grad_f(X):
        return EV_GRAD_F(X, k_init, theta_init, n_agents)
    
    def eval_g(X):
        return EV_G(X, k_init, theta_init, n_agents)
        
    def eval_jac_g(X, flag):
        return EV_JAC_G(X, flag, k_init, theta_init, n_agents)
