...
#     The crucial part is
#
#     solveriter.iterate(local_aPoints[iI], theta_init, n_agents, valold_list)[0]
...
def sparse_grid_iter(n_agents, valold_list):
    ...
    if (comm_switch!=0 or size<ntheta):
        ...
        for itheta in range(ntheta):    
            ...
            gridp=open("grid_"+str(theta_init)+"_"+str(rank)+".txt", 'a')
            ...
            file=open("comparison1_"+str(theta_init)+"_.txt", 'a')
            for iI in range(nump):
                local_aVals[iI]=solveriter.iterate(local_aPoints[iI], theta_init, n_agents, valold_list)[0]
        ...
        gridp=open("grid1_"+str(theta_init)+"_"+str(theta_rank)+".txt", 'a')
        ...
        file=open("comparison1_"+str(theta_init)+"_.txt", 'a')
        for iI in range(nump):
            local_aVals[iI]=solveriter.iterate(theta_aPoints[iI], theta_init, n_agents, valold_list)[0]
        ...
        if theta_rank==0:
            ...
            theta_grid.write("theta_grid1_"+str(color)+".txt")
        
        comm.Barrier()
        if rank==0:
            for th in range(ntheta):
                grid_list.append(TasmanianSG.TasmanianSparseGrid())
                grid_list[th].read("theta_grid1_"+str(th)+".txt")        
